...
1. **Every function must have a JSDoc or header comment** that explains:
...
```
📝 Comments Check: path/to/file.ts
═══════════════════════════════════════

🔴 Coverage Issues (X found)
  Line 42 — Function "calculateTotal" has no comment explaining what it does
  Line 58 — Complex sort logic has no inline comment
  → Overall ratio: ~1:10 (needs more comments)

🔴 Accuracy Issues (X found)
  Line 35 — Comment says "returns an array" but code returns an object
  Line 72 — Comment mentions "userId" param which no longer exists

🔴 Beginner-Friendliness Issues (X found)
  Line 18 — Uses jargon "memoize" without explanation
  Line 53 — "Hydrate the store" — unclear what "hydrate" means
